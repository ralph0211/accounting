...
Partial Class Banking_Withdrawals
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Try
 Dim bal As double= Convert.ToDouble(lblBranchBalance.Text)
            Dim cmp As double=Convert.ToDouble(txtAmount.Text)
            If cmp > bal Then
                txtAmount.Text = ""
                txtAmount.Focus()
                msgbox("The amount  must be less than or equal to Branch Balance")
                Exit Sub
            End If
        Catch ex As Exception
               msgbox("The amount  must be less than or equal to Branch Balance")
        ...
        Try
            ...
            ElseIf Trim(cmbLoanOfficerAccount.SelectedValue) = "" Then
                notify("Select the loan officer account", "error")
            ...
            ElseIf Not IsNumeric(toMoney(txtAmount.Text)) Or toMoney(txtAmount.Text) <= 0 Then
                ...
                txtAmount.Focus()
            ElseIf toMoney(lblBranchBalance.Text) <= 0 Or toMoney(txtAmount.Text) > toMoney(lblBranchBalance.Text) Then
            ...
            ElseIf Not IsDate(txtTrxnDate.Text) Then
                ...
                txtTrxnDate.Focus()
            Else
                If saveTransaction("Withdrawal", "", txtComment.Text, 0, txtAmount.Text, cmbBranchAccount.SelectedValue, cmbLoanOfficerAccount.SelectedValue, "1", "", "", "", "", txtTrxnDate.Text) Then
