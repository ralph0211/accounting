...
Partial Class ParaCreditProducts
            ...
            Else
                Using cmd As New SqlCommand("SaveCreditProduct")
                    ...
                    Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                        ...
                        cmd.Parameters.AddWithValue("@MinAmt", validateNumeric(toMoney(txtMinAmt.Text)))
                        cmd.Parameters.AddWithValue("@MaxAmt", validateNumeric(toMoney(txtMaxAmt.Text)))
                        cmd.Parameters.AddWithValue("@MinIntRate", validateNumeric(txtMinimumIntRate.Text))
                        cmd.Parameters.AddWithValue("@MaxIntRate", validateNumeric(txtMaximumIntRate.Text))
                        ...
                        cmd.Parameters.AddWithValue("@RepaymentIntervalNum", validateNumeric(txtRepaymentInterval.Text))
                        cmd.Parameters.AddWithValue("@RepaymentIntervalUnit", cmbRepaymentInterval.SelectedValue)
                        cmd.Parameters.AddWithValue("@MinimumTenure", validateNumeric(txtMinimumTenure.Text))
                        cmd.Parameters.AddWithValue("@MaximumTenure", validateNumeric(txtMaximumTenure.Text))
                        cmd.Parameters.AddWithValue("@DefaultTenure", validateNumeric(txtDefaultTenure.Text))
                        cmd.Parameters.AddWithValue("@PenaltyRate", validateNumeric(txtPenaltyRate.Text))
                        cmd.Parameters.AddWithValue("@PenaltyInterval", cmbPenaltyInterval.SelectedValue)
                        cmd.Parameters.AddWithValue("@DateCreated", validateDate(txtCreatedDate.Text))
                        cmd.Parameters.AddWithValue("@Active", chkActive.Checked)
                        cmd.Parameters.AddWithValue("@SalaryBasedLimit", IIf(rdbSalaryBasedLimit.SelectedIndex = -1, "", rdbSalaryBasedLimit.SelectedValue))
                        cmd.Parameters.AddWithValue("@LimitBasedOn", IIf(rdbLimitBasedOn.SelectedIndex = -1, "", rdbLimitBasedOn.SelectedValue))
                        cmd.Parameters.AddWithValue("@MaxRateAllowed", IIf(IsNumeric(txtLimitMaxRate.Text), txtLimitMaxRate.Text, 0))
                        cmd.Parameters.AddWithValue("@MinGracePerLength", IIf(IsNumeric(txtMinGracePerLength.Text), txtMinGracePerLength.Text, 0))
                        cmd.Parameters.AddWithValue("@MaxGracePerLength", IIf(IsNumeric(txtMaxGracePerLength.Text), txtMaxGracePerLength.Text, 0))
                        con.Open()
                        If cmd.ExecuteNonQuery() Then
    ...
    Protected Sub getProducts()
        Try
            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                Using cmd As New SqlCommand("Select a.ID,Product as [Product Type],DisplayName as [Display Name],format(MinAmt,'c') as [Minimum Amount],format(MaxAmt,'c') as [Maximum Amount] FROM [CreditProducts] a join [ParaProductTypes] b on a.ProductType=convert(varchar,b.id)")
            ...
            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                Using cmd As New SqlCommand("Select *,convert(varchar,DateCreated,106) as DateCreated1 FROM [CreditProducts] where [id]='" & productID & "'")
                    ...
                    If ds.Tables(0).Rows.Count > 0 Then
                        ...
                        Try
                            txtMinAmt.Text = FormatNumber(dr("MinAmt"), 2)
                        Catch ex As Exception
                            txtMinAmt.Text = ""
                        End Try
                        Try
                            txtMaxAmt.Text = FormatNumber(dr("MaxAmt"), 2)
                        Catch ex As Exception
                            txtMaxAmt.Text = ""
                        End Try
                        Try
