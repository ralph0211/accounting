...
Partial Class Roles
    ...
    Protected Sub btn_AddRole_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_AddRole.Click
        Try
            ...
            If isUniqueRole(txt_Role.Text) Then
                ...
                Dim addCommand = "insert into MASTER_ROLES(RoleName,USER_STATUS, USER_CREATED_DATE, USER_CREATED_BY, USER_MODIFIED_BY, USER_MODIFIED_DATE,DASHBOARD) values(''" & BankString.removeSpecialCharacter(txt_Role.Text.Trim()) & "'',''" & 1 & "'',GETDATE(),''" & userId & "'',''" & userId & "'',GETDATE(),''" & rdbDashboardView.SelectedValue & "'')"
                ...
                If (x > 0) Then
                    ...
                    recordAction("btn_AddRole", "Added role: " & txt_Role.Text)
                    txt_Role.Text = ""
                    getAddedRoles()
                End If
            Else
    ...
    Protected Sub grdAddedRoles_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles grdAddedRoles.RowDeleting
        ...
        If cmd.ExecuteNonQuery Then
            ...
            recordAction("grdAddedRoles_RowDeleting", "Deleted user role: " & roleName)
        Else
    ...
    Protected Sub grdAddedRoles_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles grdAddedRoles.RowUpdating
        ...
        If cmd.ExecuteNonQuery Then
            ...
            recordAction("grdAddedRoles_RowUpdating", "Updated user role: " & RoleName)
        Else
