...
Partial Class Accounting_DebtorAccountCreation
    Inherits System.Web.UI.Page

    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Try
            Using con = New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                Using cmd = New SqlCommand("insert into DebtorAccounts (AccountName,AccountNo,AccountDesc,MainAccount) values (@AccountName,@AccountNo,@AccountDesc,@MainAccount)", con)
    ...
    Protected Sub loadAccounts()
        Try
            Using con = New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                Using cmd = New SqlCommand("select ca.*,fac.MainAccount as [MAcc],fac.AccountName as [MainAccName] from DebtorAccounts ca left JOIN tbl_FinancialAccountsCreation fac ON ca.MainAccount=fac.MainAccount", con)
    ...
    Private Sub grdAccounts_RowDeleting(sender As Object, e As GridViewDeleteEventArgs) Handles grdAccounts.RowDeleting
        Try
            ...
            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                Using cmd = New SqlCommand("delete from [DebtorAccounts] where id='" & ViewState("accEditID") & "'", con)
    ...
    Private Sub grdAccounts_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles grdAccounts.RowUpdating
        Try
            ...
            Else
                Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                    Using cmd = New SqlCommand("update [DebtorAccounts] set AccountName=@AccountName,AccountNo=@AccountNo,AccountDesc=@AccountDesc,MainAccount=@MainAccount where id='" & ViewState("accEditID") & "'", con)
