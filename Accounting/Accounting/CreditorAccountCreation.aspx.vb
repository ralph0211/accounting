...
Partial Class Accounting_CreditorAccountCreation
    Inherits System.Web.UI.Page

    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Try
            Using con = New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                Using cmd = New SqlCommand("insert into CreditorAccounts (AccountName,AccountNo,AccountDesc,MainAccount) values (@AccountName,@AccountNo,@AccountDesc,@MainAccount)", con)
    ...
    Protected Sub loadAccounts()
        Try
            Using con = New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                Using cmd = New SqlCommand("select ca.*,fac.MainAccount as [MAcc],fac.AccountName as [MainAccName] from CreditorAccounts ca left JOIN tbl_FinancialAccountsCreation fac ON ca.MainAccount=fac.MainAccount", con)
    ...
    Private Sub grdAccounts_RowDeleting(sender As Object, e As GridViewDeleteEventArgs) Handles grdAccounts.RowDeleting
        Try
            ...
            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                Using cmd = New SqlCommand("delete from [CreditorAccounts] where id='" & ViewState("accEditID") & "'", con)
    ...
    Private Sub grdAccounts_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles grdAccounts.RowUpdating
        Try
            ...
            Else
                Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                    Using cmd = New SqlCommand("update [CreditorAccounts] set AccountName=@AccountName,AccountNo=@AccountNo,AccountDesc=@AccountDesc,MainAccount=@MainAccount where id='" & ViewState("accEditID") & "'", con)
