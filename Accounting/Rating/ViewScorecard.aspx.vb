...
Partial Class Rating_ViewScorecard
            ...
            If Not IsPostBack Then
                Try
                    ...
                    If loanID <> "" Then
                        ...
                        'lblReturn.Text = "<a href='../Credit/ApplicationApproval.aspx?id=" & loanID & "'>Return to Loan Approval</a>"
                    Else
                        'lblReturn.Text = ""
                    End If
                Catch ex As Exception
    ...
    Protected Sub btnSearchLoanID_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchLoanID.Click
        Try
            ...
            cmd = New SqlCommand("select * from QUEST_APPLICATION qa join [APPLICATION_OVERALL_RATING] aor on qa.CUSTOMER_NUMBER=aor.CLIENT_ID where qa.ID='" & Trim(txtLoanID.Text) & "'", con)
            Dim ds As New DataSet
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "LOANS")
            If ds.Tables(0).Rows.Count > 0 Then
                repSystemDecision.DataSource = Nothing
                ...
                lblViewReport.Text = "<a href='../Credit/rptRatingStatement.aspx?LOANID=" & txtLoanID.Text & "' target='new'>View Report</a>"
            Else
                lblClientName.Text = ""
                lblClientType.Text = ""
                lblLoanAmt.Text = ""
    ...
    Protected Sub btnSearchClientID_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchClientID.Click
        Try
            lblOverallRating.Text = ""
            cmd = New SqlCommand("select * from QUEST_APPLICATION qa join [APPLICATION_OVERALL_RATING] aor on qa.CUSTOMER_NUMBER=aor.CLIENT_ID where qa.CUSTOMER_NUMBER='" & Trim(txtClientID.Text) & "'", con)
            Dim ds As New DataSet
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "LOANS")
            If ds.Tables(0).Rows.Count > 0 Then
    ...
    ''    Try
    ''        cmd = New SqlCommand("select *,description as CATEGORY from [entity_scoring_categories] where [entity_id]='2'", con)
    ''        Dim ds As New DataSet
    ''        adp = New SqlDataAdapter(cmd)
    ...
    '    Try
    '        cmd = New SqlCommand("select *,[description] as QUESTION from [entity_scoring_variables] where [category_id]='" & catID & "'", con)
    '        Dim ds As New DataSet
    '        adp = New SqlDataAdapter(cmd)
    ...
    '        cmd = New SqlCommand("select [id],[variable_id],[lower_range],[upper_range],[score],[scale],isnull(lower_range + ' - ' + upper_range,lower_range) as [comment] from rating_ranges where [variable_id]='" & questID & "'", con)
    '        Dim ds As New DataSet
    '        adp = New SqlDataAdapter(cmd)
    '        adp.Fill(ds, "PRV")
    '        If ds.Tables(0).Rows.Count > 0 Then
    '            rdbX.DataSource = ds.Tables(0)
    '            rdbX.DataValueField = "score"
    ...
    '        Dim cmd1 = New SqlCommand("select max(isnull(score,0)) from [rating_ranges]", con)
    '        If con.State = ConnectionState.Open Then
    '            con.Close()
    '        End If
    '        con.Open()
    ...
    '            Else
    '                cmd = New SqlCommand("insert into APPLICATION_OVERALL_RATING ([LOAN_ID],[CLIENT_ID],[AVERAGE_RATING],[RATED_BY],[RATED_DATE]) values ('" & loanID & "','" & ds.Tables(0).Rows(0).Item("CLIENT_ID") & "','" & convertToSaveFormat(overRating) & "','" & Session("UserID") & "','" & DateFormat.getSaveDateTime(Date.Now) & "')", con)
    '            End If
    ...
    Protected Sub btnSearchName_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearchName.Click
        Try
            ...
            cmd = New SqlCommand("select qa.ID,SURNAME+' '+FORENAMES+' '+convert(varchar,CUSTOMER_NUMBER)+' '+convert(varchar,FIN_AMT) as DISPLAY from QUEST_APPLICATION qa join [APPLICATION_OVERALL_RATING] aor on qa.ID=aor.LOAN_ID where DISBURSED='1' AND SURNAME like '" & txtSearchName.Text & "%'", con)
            Dim ds As New DataSet
            Dim adp As New SqlDataAdapter
            adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "LOANS")
