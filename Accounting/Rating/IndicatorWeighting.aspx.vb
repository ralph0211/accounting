...
Partial Class Rating_IndicatorWeighting
    ...
    Protected Sub getCategories(ByVal entityID As String)
        ...
        Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
            Using sda As New SqlDataAdapter()
                ...
                Dim query As String = "select distinct ROW_NUMBER() OVER(ORDER BY esc.id ASC) AS Row,esc.[sub_category] as category,esc.id as cat_id, isnull(esc.weight,0) as weight from entity_scoring_subcategories esc join entity_scoring_categories escat on esc.category_id=escat.id where esc.active='1' and escat.entity_id='" & entityID & "'"
    ...
    Protected Function getSubCategories(ByVal catID As Double) As DataTable
        ...
        Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
            Using sda As New SqlDataAdapter()
                ...
                Dim query As String = "select isnull(essc.weight,0) as weight,essc.id as subcatID, essc.description as sub_category from entity_scoring_variables essc where essc.subcategory_id='" & catID & "'"
                Dim cmd As New SqlCommand(query)
                cmd.Connection = con
                sda.SelectCommand = cmd
                Using ds As New DataSet()
        ...
        For Each category As RepeaterItem In rptCategories.Items
            ...
            If subWeight = 100 Then
                ...
                For Each dr As DataRow In dt.Rows
                    commitTemporaryWeight(dr("variable_id"), dr("weight"))
                Next
    ...
    Protected Sub saveTemporaryWeight(subCatID As String, wgt As Double)
        Dim cmd = New SqlCommand("insert into temp_variable_wgt values('" & subCatID & "','" & wgt & "')", con)
    ...
    Protected Sub deleteTemporaryWeight()
        Dim cmd = New SqlCommand("delete from temp_variable_wgt", con)
        If con.State <> ConnectionState.Closed Then
            con.Close()
        End If
        con.Open()
    ...
    Protected Function getTemporaryWeight() As DataTable
        Dim cmd = New SqlCommand("select * from temp_variable_wgt", con)
    ...
    Protected Sub commitTemporaryWeight(subCatID As String, wgt As Double)
        Dim cmd = New SqlCommand("update entity_scoring_variables set weight='" & wgt & "' where id='" & subCatID & "'", con)
        If con.State <> ConnectionState.Closed Then
            con.Close()
        End If
        con.Open()
