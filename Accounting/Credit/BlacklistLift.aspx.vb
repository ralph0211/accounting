...
Partial Class QuestCredit_BlacklistLift
    ...
    Protected Sub btnSaveName_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveName.Click
        Try
            If Trim(txtCustNo.Text) = "" Then
                notify("Select the client to lift from blacklist", "error")
            ElseIf Trim(txtBlacklistLiftReason.Text) = "" Then
                notify("Enter the reason for lifting this client", "error")
                txtBlacklistLiftReason.Focus()
            ElseIf Trim(txtDateLifted.Text) = "" Then
                notify("Enter the date when client was lifted", "error")
                txtDateLifted.Focus()
            Else
                liftBlacklist(ViewState("BlacklistID"))
            End If
    ...
    Protected Sub getNames()
        Try
            'cmd = New SqlCommand("select ID as [orderID], CUSTOMER_NUMBER as [ID],SURNAME,FORENAMES,ADDRESS from CUSTOMER_DETAILS where [CUSTOMER_TYPE]='Individual' and [Blacklisted]=1 order by orderID DESC", con)
            Using cmd = New SqlCommand("select ID, CustNo as [Customer Number],Surname,Forenames,convert(varchar,[BlacklistDate],113) as [Blacklist Date],[BlacklistedBy] as [Blacklisted By],[BlacklistReason] as [Reason] from Blacklist where [Blacklisted]=1 and [BlacklistAuthorized]=1 and (LiftAuthorized=0 or LiftAuthorized is null) order by ID DESC", con)
    ...
    Protected Sub getFarmers()
        Try
            cmd = New SqlCommand("select ID as [orderID], CUSTOMER_NUMBER as [ID],SURNAME as [Group Name],FORENAMES as [Farmer Name],ADDRESS from CUSTOMER_DETAILS where [CUSTOMER_TYPE]='Farmer' and [Blacklisted]=1 order by orderID DESC", con)
            Dim ds As New DataSet
    ...
    Protected Sub getGroups()
        Try
            cmd = New SqlCommand("select ID as [orderID], CUSTOMER_NUMBER as [ID],SURNAME as [Group Name] from CUSTOMER_DETAILS where [CUSTOMER_TYPE]='Group' and [Blacklisted]=1 order by orderID DESC", con)
            Dim ds As New DataSet
            Using adp = New SqlDataAdapter(cmd)
                adp.Fill(ds, "CUSTOMER")
            End Using
    ...
    Protected Sub getNamesBySurname(ByVal surname As String)
        Try
            'cmd = New SqlCommand("select ID as [orderID], CUSTOMER_NUMBER as [ID],SURNAME,FORENAMES,ADDRESS from CUSTOMER_DETAILS where [CUSTOMER_TYPE]='Individual' and SURNAME + ' ' + FORENAMES like '" & surname & "%' and [Blacklisted]=1 order by orderID DESC", con)
            cmd = New SqlCommand("select ID, CustNo as [Customer Number],Surname,Forenames,convert(varchar,[BlacklistDate],113) as [Blacklist Date],[BlacklistedBy] as [Blacklisted By],[BlacklistReason] as [Reason] from Blacklist where [Blacklisted]=1 and SURNAME + ' ' + FORENAMES like '" & surname & "%' and [BlacklistAuthorized]=1 and (LiftAuthorized=0 or LiftAuthorized is null) order by ID DESC", con)
            Dim ds As New DataSet
            Dim adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "CUSTOMER")
            If ds.Tables(0).Rows.Count > 0 Then
    ...
    Protected Sub getFarmersByName(ByVal name As String)
        Try
            cmd = New SqlCommand("select ID as [orderID], CUSTOMER_NUMBER as [ID],SURNAME as [Group Name],FORENAMES as [Farmer Name],ADDRESS from CUSTOMER_DETAILS where [CUSTOMER_TYPE]='Farmer' and FORENAMES + ' ' + SURNAME like '%" & name & "%' and [Blacklisted]=1 order by orderID DESC", con)
            Dim ds As New DataSet
            Dim adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "CUSTOMER")
            If ds.Tables(0).Rows.Count > 0 Then
    ...
    Protected Sub getGroupsByName(ByVal name As String)
        Try
            cmd = New SqlCommand("select ID as [orderID], CUSTOMER_NUMBER as [ID],SURNAME as [Group Name] from CUSTOMER_DETAILS where [CUSTOMER_TYPE]='Group' and SURNAME like '%" & name & "%' and [Blacklisted]=1 order by orderID DESC", con)
            Dim ds As New DataSet
            Dim adp = New SqlDataAdapter(cmd)
            adp.Fill(ds, "CUSTOMER")
            If ds.Tables(0).Rows.Count > 0 Then
    ...
    Protected Sub grdNames_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdNames.SelectedIndexChanged
        ViewState("BlacklistID") = grdNames.SelectedRow.Cells(1).Text
        getNamesForEdit(grdNames.SelectedRow.Cells(2).Text)
    End Sub
    ...
    Protected Sub btnSaveFarmer_Click(sender As Object, e As EventArgs) Handles btnSaveFarmer.Click
        Try
            If Trim(txtCustNo.Text) = "" Then
                notify("Select the client to blacklist", "error")
            ElseIf Trim(txtBlacklistLiftReason.Text) = "" Then
                notify("Enter the reason for lifting this client from blacklist", "error")
                txtBlacklistReason.Focus()
            ElseIf Trim(txtDateLifted.Text) = "" Then
                notify("Enter the date when client was lifted from blacklist", "error")
    ...
    Protected Sub liftBlacklist(blID As String)
        Try
            Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("Constring").ConnectionString)
                Using cmd = New SqlCommand("update Blacklist set Lifted=1,LiftDate=@LiftDate,LiftedBy=@LiftedBy,LiftTimeStamp=GETDATE(),LiftReason=@LiftReason where id=@id", con)
                    cmd.Parameters.AddWithValue("@LiftedBy", Session("UserId"))
                    cmd.Parameters.AddWithValue("@id", blID)
                    cmd.Parameters.AddWithValue("@LiftReason", txtBlacklistLiftReason.Text)
                    cmd.Parameters.AddWithValue("@LiftDate", txtDateLifted.Text)
                    If con.State = ConnectionState.Open Then
                        con.Close()
                    End If
                    con.Open()
                    If cmd.ExecuteNonQuery Then
                        notify("Blacklist lifted. Authorization pending", "success")
                        grdNames.SelectedIndex = -1
                        clearAll()
                        txtBlacklistLiftReason.Text = ""
                        txtDateLifted.Text = ""
                        getNames()
                    Else
                        notify("Error uplifting client from blacklist", "error")
                    End If
                    con.Close()
                End Using
            End Using
        Catch ex As Exception
            WriteLogFile(Session("UserId"), Request.Url.ToString & " --- liftBlacklist()", ex.ToString)
        End Try
    End Sub
    ...
    'Private Sub grdNames_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles grdNames.RowCommand
    '    Try
    ...
    '        getNamesForEdit(csn)
    '    Catch ex As Exception
    '        WriteLogFile(Session("UserId"), Request.Url.ToString & " - grdNames_RowCommand()", ex.Message)
    '    End Try
    'End Sub
End Class